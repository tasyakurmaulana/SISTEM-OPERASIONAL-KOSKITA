...
$username = ($query_user && $query_user->num_rows > 0) ? $query_user->fetch_assoc()['username'] : 'Admin';
$nama_laundry = ucfirst($username);

$query_aktif = $conn->query("SELECT COUNT(id_laundry) as total_aktif FROM pesanan_laundry WHERE status IN ('Menunggu', 'Diproses')");
$pesanan_aktif = ($query_aktif && $query_aktif->num_rows > 0) ? $query_aktif->fetch_assoc()['total_aktif'] : 0;
...
<body class="bg-gray-50 font-sans antialiased">
    ...
    <nav class="bg-cyan-600 text-white shadow-md">
        <div class="max-w-4xl mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <a href="../logout.php" class="text-sm bg-red-500 hover:bg-red-600 px-4 py-2 rounded-lg transition font-medium">Keluar</a>
        </div>
    </nav>
    ...
    <div class="max-w-4xl mx-auto px-4 py-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 mb-6 flex items-center justify-between border border-gray-100">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Halo, <?php echo htmlspecialchars($nama_laundry); ?>! 🫧</h1>
            ...
            <div class="w-14 h-14 bg-cyan-100 text-cyan-700 rounded-full flex items-center justify-center text-xl font-bold shadow-sm border border-cyan-200">
                <?php echo strtoupper(substr($nama_laundry, 0, 1)); ?>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <a href="ambil_pesanan.php" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center hover:shadow-md hover:border-cyan-300 transition group relative">
                <?php
                    $q_menunggu = $conn->query("SELECT COUNT(id_laundry) as jml FROM pesanan_laundry WHERE status = 'Menunggu'");
                    $jml_menunggu = ($q_menunggu) ? $q_menunggu->fetch_assoc()['jml'] : 0;
                    if($jml_menunggu > 0):
                ?>
                    <span class="absolute top-3 right-3 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full animate-bounce">
                        <?php echo $jml_menunggu; ?> BARU
                    </span>
                <?php endif; ?>
                ...
                <span class="text-base font-bold text-gray-700 text-center">Ambil Cucian</span>
                <p class="text-xs text-gray-400 text-center mt-1">Pesanan masuk</p>
            </a>

            <a href="daftar_pesanan.php" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center hover:shadow-md hover:border-blue-300 transition group relative">
                <?php
                    $q_proses = $conn->query("SELECT COUNT(id_laundry) as jml FROM pesanan_laundry WHERE status = 'Diproses'");
                    $jml_proses = ($q_proses) ? $q_proses->fetch_assoc()['jml'] : 0;
                    if($jml_proses > 0):
                ?>
                    <span class="absolute top-3 right-3 bg-blue-500 text-white text-[10px] font-bold px-2 py-1 rounded-full">
                        <?php echo $jml_proses; ?> PROSES
                    </span>
                <?php endif; ?>
                ...
                <span class="text-base font-bold text-gray-700 text-center">Kelola Cucian</span>
                <p class="text-xs text-gray-400 text-center mt-1">Timbang & Lunas</p>
            </a>
